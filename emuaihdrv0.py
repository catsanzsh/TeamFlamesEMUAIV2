...
class MIPSR4300i:
    """Enhanced MIPS R4300i CPU emulation with better ROM compatibility"""
    def __init__(self, memory):
        self.memory = memory
        self.pc = 0xBFC00000  # Boot ROM vector - proper N64 boot sequence
        self.regs = [0] * 32
        ...
        # Initialize CP0 registers for proper boot
        self.cp0_regs[12] = 0x10400000  # Status register - kernel mode
        self.cp0_regs[15] = 0x00000B00  # PRId - R4300i processor ID
        
    def reset(self):
        """Reset CPU to initial state for ROM loading"""
        ...
        self.regs[29] = 0x801FFFF0  # $sp - stack pointer
    ...
    def execute(self):
        ...
        if instruction == 0:  # NOP or invalid instruction
            self.pc += 4
            return
        ...
        def sign_extend_8(value):
            return value | (0xFFFFFF00 if (value & 0x80) else 0)

        # Enhanced instruction set for better ROM compatibility
